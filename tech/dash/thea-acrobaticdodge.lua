...
function init()
  ...
  self.rollCycle = config.getParameter("rollCycle")
  ...
  self.rechargeDirectives = config.getParameter("rechargeDirectives", "?fade=FFFFFF=0.25")
...
function update(args)
  ...
  if self.dashTimer > 0 then
	...
	mcontroller.setRotation(-math.pi * 2 * self.dashDirection * (-self.dashTimer / self.dashDuration * self.rollCycle))
