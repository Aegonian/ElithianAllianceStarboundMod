...
function build(directory, config, parameters, level, seed)
  ...
  config.miningStrengthLevelMultiplier = root.evalFunction("thea-chainsawMiningStrengthTimeMultiplier", configParameter("level", 1))
  ...
  if config.tooltipKind ~= "base" then
    ...
    config.tooltipFields.energyPerSecondLabel = util.round(config.primaryAbility.energyUsage or 0)
    config.tooltipFields.miningStrengthLabel = util.round((config.primaryAbility.impactTileDamageForeground or config.primaryAbility.impactTileDamageBackground or 1.0) * config.miningStrengthLevelMultiplier, 1)
