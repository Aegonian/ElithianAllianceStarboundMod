...
function build(directory, config, parameters, level, seed)
  ...
  if config.tooltipKind ~= "base" then
	...
	if config.primaryAbility.stances.preReloadTwirl then
	  config.tooltipFields.reloadTimeLabel = util.round(config.primaryAbility.stances.reload.duration + (config.primaryAbility.stances.preReloadTwirl.duration or 0) + config.primaryAbility.readyTime, 1)
	elseif config.primaryAbility.stances.reloadTwirl then
