...
function build(directory, config, parameters, level, seed)
  ...
  if config.tooltipKind ~= "base" then
	...
	config.tooltipFields.chargedShotDamageLabel = util.round(config.primaryAbility.baseDamage * config.damageLevelMultiplier * config.primaryAbility.chargePowerMultiplier, 1)
