...
function build(directory, config, parameters, level, seed)
  ...
  if config.tooltipKind ~= "base" then
	...
	config.tooltipFields.shotsToOverheatLabel = util.round(config.primaryAbility.overheatThreshold / config.primaryAbility.heatPerShot, 1)
	config.tooltipFields.cooldownTimeLabel = util.round(config.primaryAbility.overheatThreshold / config.primaryAbility.heatLossRateOverheated, 1)
