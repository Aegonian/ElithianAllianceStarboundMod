...
function build(directory, config, parameters, level, seed)
  ...
  if config.tooltipKind ~= "base" then
    ...
    config.tooltipFields.minSpeedLabel = util.round(1 / (config.primaryAbility.minFiringSpeed or 1.0), 1)
	config.tooltipFields.maxSpeedLabel = util.round(1 / (config.primaryAbility.maxFiringSpeed or 1.0), 1)
	config.tooltipFields.chargeTimeLabel = util.round(config.primaryAbility.chargeTime, 1)
	config.tooltipFields.resetTimeLabel = util.round(config.primaryAbility.resetTime, 1)
