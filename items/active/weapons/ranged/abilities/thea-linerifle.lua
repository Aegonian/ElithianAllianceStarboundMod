...
-- Bow primary ability
TheaLineRifle = WeaponAbility:new()

function TheaLineRifle:init()
...
function TheaLineRifle:update(dt, fireMode, shiftHeld)
...
function TheaLineRifle:fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
...
function TheaLineRifle:setChargeLevel(chargePercentage, currentLevel)
...
function TheaLineRifle:setLightning(amount, width, forks, displacement, color, startOffset, endOffset)
  local lightning = {}
  for i = 1, amount do
    local bolt = {
...
function TheaLineRifle:muzzleFlash()
...
function TheaLineRifle:cooldown()
...
function TheaLineRifle:uninit()
  self:reset()
end

function TheaLineRifle:reset()
  animator.setAnimationState("charge", "off")
