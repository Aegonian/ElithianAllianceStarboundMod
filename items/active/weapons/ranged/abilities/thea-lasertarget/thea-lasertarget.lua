...
-- Bow primary ability
TheaLaserTarget = WeaponAbility:new()

function TheaLaserTarget:init()
...
function TheaLaserTarget:update(dt, fireMode, shiftHeld)
...
function TheaLaserTarget:charge()
  ...
  while self.chargeTimer > 0 and self.fireMode == (self.activatingFireMode or self.abilitySlot) and not world.lineTileCollision(mcontroller.position(), self:firePosition()) do
	...
	--Tell the laser to use the default colour
    activeItem.setScriptedAnimationParameter("laserColour", self.laserColourPrimary)
	
	--Change the laser colour when nearly done charging
	if self.chargeTimer < 0.1 then
	  activeItem.setScriptedAnimationParameter("laserColour", self.laserColourSecondary)
	end
...
function TheaLaserTarget:fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
  ...
  for i = 1, (projectileCount or self.projectileCount) do
    ...
    projectileId = world.spawnProjectile(
        projectileType,
        firePosition or self:targetPosition(),
...
function TheaLaserTarget:muzzleFlash()
...
function TheaLaserTarget:cooldown()
  --Use the inactive laser colour (usually invisible)
  activeItem.setScriptedAnimationParameter("laserColour", self.laserColourInactive)
...
function TheaLaserTarget:firePosition()
  return vec2.add(mcontroller.position(), activeItem.handPosition(self.weapon.muzzleOffset))
end

function TheaLaserTarget:targetPosition()
