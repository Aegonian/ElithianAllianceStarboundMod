...
-- Bow primary ability
TheaLineRifleSwarmFire = WeaponAbility:new()

function TheaLineRifleSwarmFire:init()
  ...
  self.primaryProjectile = nil
  self.secondaryProjectiles = {}
end

function TheaLineRifleSwarmFire:update(dt, fireMode, shiftHeld)
...
function TheaLineRifleSwarmFire:charge()
...
function TheaLineRifleSwarmFire:fire()
...
function TheaLineRifleSwarmFire:fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
  ...
  for i = 1, (projectileCount or self.projectileCount) do
    ...
    projectileId = world.spawnProjectile(
	  self.projectileTypes[shotNumber],
	...
	--If this was the first projectile, set it as the swarm target
	if shotNumber == 1 then
	  self.primaryProjectile = projectileId
	--Otherwise, add this projectile to the list of swarmers
	else
	  table.insert(self.secondaryProjectiles, projectileId)
	end
  end
  
  --Send the IDs of the swarmers to the primary projectile
  if self.primaryProjectile ~= nil then
	world.callScriptedEntity(self.primaryProjectile, "setSwarmers", self.secondaryProjectiles)
  end
  return projectileId
end

function TheaLineRifleSwarmFire:setChargeLevel(chargePercentage, currentLevel)
...
function TheaLineRifleSwarmFire:setLightning(amount, width, forks, displacement, color, startOffset, endOffset)
  local lightning = {}
  for i = 1, amount do
    local bolt = {
...
function TheaLineRifleSwarmFire:muzzleFlash()
...
function TheaLineRifleSwarmFire:cooldown()
...
function TheaLineRifleSwarmFire:firePosition()
  return vec2.add(mcontroller.position(), activeItem.handPosition(self.weapon.muzzleOffset))
end

function TheaLineRifleSwarmFire:aimVector(inaccuracy, shotNumber)
...
function TheaLineRifleSwarmFire:energyPerShot()
  return self.baseEnergyUsage * (self.energyUsageMultiplier or 1.0)
end

function TheaLineRifleSwarmFire:damagePerShot()
...
function TheaLineRifleSwarmFire:reset()
  animator.setAnimationState("chargeAlt", "off")
