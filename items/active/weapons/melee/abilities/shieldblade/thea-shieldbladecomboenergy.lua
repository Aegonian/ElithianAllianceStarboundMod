...
TheaShieldBladeComboEnergy = WeaponAbility:new()

function TheaShieldBladeComboEnergy:init()
  self.comboStep = 1
  ...
  self.wasActiveLastFrame = false
  self.energyActive = false
...
function TheaShieldBladeComboEnergy:update(dt, fireMode, shiftHeld)
  WeaponAbility.update(self, dt, fireMode, shiftHeld)
  
  --Setting up the energy blade system
  self.energyActive = self.weapon.currentAbility ~= nil
  if self.energyActive == true then
	if fireMode ~= "alt" then
	  if self.wasActiveLastFrame == false then
		animator.setAnimationState("weapon", "activate")
		self.wasActiveLastFrame = true
	  end
	end
	self.activeTimer = self.activeTime
  else
	self.activeTimer = math.max(0, self.activeTimer - self.dt)
	if self.activeTimer == 0 and animator.animationState("weapon") ~= "idle" then
	  animator.setAnimationState("weapon", "stop")
	  self.wasActiveLastFrame = false
	end
  end
...
-- State: windup
function TheaShieldBladeComboEnergy:windup()
...
function TheaShieldBladeComboEnergy:wait()
...
function TheaShieldBladeComboEnergy:preslash()
...
-- State: fire
function TheaShieldBladeComboEnergy:fire()
...
function TheaShieldBladeComboEnergy:shouldActivate()
...
function TheaShieldBladeComboEnergy:readyFlash()
  animator.setGlobalTag("bladeDirectives", self.flashDirectives)
  self.flashTimer = self.flashTime
end

function TheaShieldBladeComboEnergy:computeDamageAndCooldowns()
  local attackTimes = {}
...
function TheaShieldBladeComboEnergy:uninit()
