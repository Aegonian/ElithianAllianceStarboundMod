...
function update()
  ...
  --Return guns to idle states when not firing
  else
  ...
  if driverThisFrame then
	...
	if self.facingDirection < 0 then
	  if self.aimAngle > 0 then
		self.aimAngle = math.max(self.aimAngle, math.pi - self.aimLimit)
		self.fireAngle = math.max(self.fireAngle, math.pi - self.aimLimit)
	  else
		self.aimAngle = math.min(self.aimAngle, -math.pi + self.aimLimit)
		self.fireAngle = math.min(self.fireAngle, -math.pi + self.aimLimit)
	  end
	  animator.rotateGroup("guns", math.pi - self.aimAngle)
	else
	  if self.aimAngle > 0 then
        self.aimAngle = math.min(self.aimAngle, self.aimLimit)
        self.fireAngle = math.min(self.fireAngle, self.aimLimit)
      else
        self.aimAngle = math.max(self.aimAngle, -self.aimLimit)
        self.fireAngle = math.max(self.fireAngle, -self.aimLimit)
      end	  
	...
	if vehicle.controlHeld("drivingSeat", "primaryFire") then
	  if self.fireTimer <= 0 then
		--local fireAngle = {math.cos(self.aimAngle), math.sin(self.aimAngle)}
	...
	local foregroundGunPosition = vec2.add(mcontroller.position(), animator.partPoint("foregroundgun", "firePoint"))
	local backgroundGunPosition = vec2.add(mcontroller.position(), animator.partPoint("backgroundgun", "firePoint"))
	world.debugPoint(foregroundGunPosition, "red")
	world.debugPoint(backgroundGunPosition, "red")
	
	--Debug firing angles
	local foregroundAimAngle = vec2.add(foregroundGunPosition, vec2.rotate({1, 0}, self.fireAngle))
	local backgroundAimAngle = vec2.add(backgroundGunPosition, vec2.rotate({1, 0}, self.fireAngle))
	world.debugLine(foregroundGunPosition, foregroundAimAngle, "red")
	world.debugLine(backgroundGunPosition, backgroundAimAngle, "red")
  else
...
function animate(driverThisFrame)
  if driverThisFrame then
	...
	if vehicle.controlHeld("drivingSeat", "jump") then
	  animator.setAnimationState("thrusters", "boost")
	elseif vehicle.controlHeld("drivingSeat", "up") or vehicle.controlHeld("drivingSeat", "left") or vehicle.controlHeld("drivingSeat", "right") then
	  animator.setAnimationState("thrusters", "active")
	else
	  animator.setAnimationState("thrusters", "idle")
	end
  else
	--Thruster animation control
	animator.setAnimationState("thrusters", "invisible")
