...
function init()
  ...
  self.liquidCheckPosition = vehicle.configParameter("liquidCheckPosition")
  self.liquidCheckPositionForRotation = vehicle.configParameter("liquidCheckPositionForRotation")
  self.hoverOverLiquidDistance = vehicle.configParameter("hoverOverLiquidDistance")
  self.isHoveringOverLiquid = false
  ...
  animator.setGlobalTag("leftThrusterFrame", 1)
  animator.setGlobalTag("rightThrusterFrame", 1)

  animator.setAnimationState("leftThruster", "off")
  animator.setAnimationState("rightThruster", "off")
...
function updateDriveEffects(healthFactor, driverThisFrame)
  ...
  local leftThrusterFrame = 0
  local rightThrusterFrame = 0

  -- is the engine sound playing ?
  if (self.loopPlaying~=nil) then

    if (self.engineVolume == self.engineIdleVolume) then
      animator.setParticleEmitterActive("leftThrusterIdle", true)
      animator.setParticleEmitterActive("leftThrusterJump", false)
    else
      animator.setParticleEmitterActive("leftThrusterIdle", true)
      animator.setParticleEmitterActive("leftThrusterJump", true)
      leftThrusterFrame = 3
    end
    ...
    animator.setAnimationState("leftThruster", "on")
    animator.setAnimationState("rightThruster", "on")

  else
    --animator.setParticleEmitterActive("leftThrusterIdle", false)
    --animator.setParticleEmitterActive("leftThrusterJump", false)
    --animator.setParticleEmitterActive("ventralThrusterIdle", false)
    --animator.setParticleEmitterActive("ventralThrusterJump", false)
  ...
  --if burning, take dammage intermittantly.
  ...
  leftThrusterFrame = leftThrusterFrame+math.random(3)
  animator.setGlobalTag("leftThrusterFrame", leftThrusterFrame)
  
  rightThrusterFrame = rightThrusterFrame+math.random(3)
  animator.setGlobalTag("rightThrusterFrame", rightThrusterFrame)
...
function move()
  ...
  if self.driver then
	...
	--Hover above liquid
	--Check if there is a liquid at the designated position
	local liquidPoint = self.liquidCheckPosition
	liquidPoint = vec2.rotate(self.liquidCheckPosition, self.angle)
	liquidPoint = vec2.add(self.liquidCheckPosition, mcontroller.position())
	world.debugPoint(liquidPoint, {0, 255, 255, 255})
	
	--If true, manually set the ground distance to trick the bike into thinking the ground is closer so it will hover
	if world.liquidAt(liquidPoint) then
	  groundDistance = self.hoverOverLiquidDistance
	end
	
	--Check if there is a liquid at a slightly lower designated position, used for straightening out the rotation later
	local liquidPointForRotation = self.liquidCheckPositionForRotation
	liquidPointForRotation = vec2.rotate(self.liquidCheckPositionForRotation, self.angle)
	liquidPointForRotation = vec2.add(self.liquidCheckPositionForRotation, mcontroller.position())
	world.debugPoint(liquidPointForRotation, {0, 255, 255, 255})
	
	--If true, force the bike's rotation to levelled later on
	if world.liquidAt(liquidPointForRotation) then
	  self.isHoveringOverLiquid = true
	else
	  self.isHoveringOverLiquid = false
	end
  ...
  --Straigthen out vehicle if hovering over a liquid
  elseif self.isHoveringOverLiquid == true then
	local targetAngle = 0
    self.angle = self.angle + (targetAngle - self.angle) * self.angleApproachFactor
  
  --Rotate vehicle based on terrain slopes
  else	
  ...
  if nearGround or self.isHoveringOverLiquid == true then
